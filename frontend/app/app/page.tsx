...
function StatCard({
  ...
  label: string; value: string; sub?: string; loading?: boolean; tip?: string;
}) {
  return (
    <div className="stat-card">
      ...
      {loading ? (
        <>
          <Skeleton h={26} w="60%" br={5} />
          <div style={{ height: 5 }} />
          <Skeleton h={12} w="40%" br={4} />
        </>
      ) : (
        <>
          <div className="stat-num">{value}</div>
          {sub && (
            <div style={{ fontSize: 12, color: "var(--tx3)", marginTop: 4, fontWeight: 500 }}>{sub}</div>
          )}
        </>
      )}
    </div>
...
function ActionCard({
  href, emoji, title, desc, primary,
}: {
  href: string; emoji: string; title: string; desc: string; primary?: boolean;
}) {
  return (
    <Link href={href} style={{ textDecoration: "none", display: "block" }}>
      <div
        ...
        style={{
          ...
          border: primary ? "1px solid var(--teal-20)" : "1px solid var(--bdr)",
          transition: "all 0.2s",
        }}
        onMouseEnter={e => {
        ...
        onMouseLeave={e => {
          const el = e.currentTarget as HTMLDivElement;
          el.style.borderColor = primary ? "var(--teal-20)" : "var(--bdr)";
      ...
      >
        <div className="card-p" style={{ height: "100%" }}>
          ...
          <div style={{
            ...
            fontSize: 15, color: "var(--tx)", marginBottom: 6,
          }}>
            {title}
          </div>
          <div style={{ fontSize: 13, color: "var(--tx2)", lineHeight: 1.55, marginBottom: 16 }}>
            {desc}
          </div>
          <div style={{
      ...
      <div style={{ marginBottom: 26, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <h1 style={{
            ...
            color: "var(--tx)", letterSpacing: "-0.03em", marginBottom: 3,
          }}>
            {isConnected ? "Overview" : "Dashboard"}
          </h1>
          <p style={{ fontSize: 13, color: "var(--tx3)", fontWeight: 500 }}>
            {isConnected
              ? "Your payment activity on Arc Testnet."
        ...
        >
          <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
            <path d={showGlobal ? "M2 8l4-4 4 4" : "M2 4l4 4 4-4"} stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"/>
          </svg>
      ...
      {!isConnected ? (
        <div className="card" style={{ marginBottom: 20 }}>
          <EmptyState
            ...
            desc="Connect a wallet to view your batches, streams, and agent activity on Arc Testnet."
          />
        </div>
      ) : (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginBottom: 20 }}>
          ...
          <div className="stat-card">
            ...
            <div style={{
              ...
              color: "var(--teal-l)", wordBreak: "break-all", lineHeight: 1.5,
              marginBottom: 8,
            }}>
              {address}
            </div>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
          <ActionCard
            ...
            desc="Send USDC to up to 500 recipients in a single transaction. Import CSV or add manually."
            primary
          />
          <ActionCard
            href="/app/streams"
            ...
            desc="Linear USDC vesting for payroll, contractor agreements, and token grants. Cancel anytime."
          />
          <ActionCard
            href="/app/agents"
            emoji="🤖"
            title="Agent Registry"
            desc="Register AI wallets with USDC spending caps for fully autonomous onchain commerce."
          />
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        ...
        <div className="card">
          <div className="card-hd">
            ...
            {FLUX_ADDRESS && (
              <a
                href={explorerLink("address", FLUX_ADDRESS)}
                target="_blank" rel="noopener noreferrer"
                style={{ fontFamily: "'Manrope',sans-serif", fontSize: 12, fontWeight: 600, color: "var(--teal)" }}
              >
                ArcScan ↗
              </a>
            )}
          ...
          <div className="card-p">
            {FLUX_ADDRESS ? (
              <div style={{
                ...
                color: "var(--tx2)", wordBreak: "break-all", marginBottom: 14, lineHeight: 1.6,
              }}>
                {FLUX_ADDRESS}
              </div>
            ) : (
              <div className="banner warn" style={{ marginBottom: 14 }}>
                Contract not deployed. Run: <code style={{ fontFamily: "'IBM Plex Mono',monospace" }}>npm run deploy:arc</code>
              </div>
            )}
        ...
        <div className="card">
          <div className="card-hd">
            ...
            <span className="chip chip-teal" style={{ fontSize: 10 }}>Checklist</span>
          </div>
          <div className="card-p">
            {[
              ...
              <div key={s.n} style={{ display: "flex", gap: 12, marginBottom: 11, alignItems: "flex-start" }}>
                <div style={{
                  ...
                  background: s.done ? "var(--teal-10)" : "var(--bg3)",
                  border: `1px solid ${s.done ? "var(--teal-20)" : "var(--bdr)"}`,
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, fontWeight: 700,
                  color: s.done ? "var(--teal)" : "var(--tx3)",
                }}>
                  {s.done ? "✓" : s.n}
                </div>
                <div>
                  <div style={{
                    ...
                    color: s.done ? "var(--tx3)" : "var(--tx)", marginBottom: 1,
