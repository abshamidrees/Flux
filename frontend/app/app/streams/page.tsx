...
export default function StreamsPage() {
  ...
  /* ── Value refs — update on every keystroke, read at submit ── */
  const recipVal  = useRef("");
  const amountVal = useRef("");
  const startVal  = useRef("");
  const endVal    = useRef("");

  /* Preview state (drives VestingPreview re-render only) */
  ...
  /* ── Validate at click using value refs ── */
  const handleCreateClick = () => {
    setCreateErr("");
    const recip  = recipVal.current.trim();
    const amount = amountVal.current.trim();
    const start  = startVal.current.trim();
    const end    = endVal.current.trim();
    ...
    setSnap({ recip, amount, start, end });
    setConfirm(true);
  };
  ...
  const doCreate = async () => {
    ...
    if (!FLUX_ADDRESS || !USDC_ADDRESS) {
      setCreateErr("Contracts not deployed — set NEXT_PUBLIC_FLUX_ADDRESS in .env.local");
      setBusy(false);
      return;
    }
    try {
      ...
      // Date picker gives midnight UTC — if that's in the past, use now + 2 min
      ...
      setCreateErr((e as { shortMessage?: string }).shortMessage?.slice(0, 140) || "Transaction failed");
    } finally { setBusy(false); }
  };

  const handleWithdrawClick = () => {
    setWErr("");
    const id = wIdVal.current.trim();
  ...
  const doWithdraw = async () => {
    ...
    setWBusy(true);
    try {
      const tx = await writeContractAsync({ address: FLUX_ADDRESS as `0x${string}`, abi: FLUX_ABI, functionName: "withdrawFromStream", args: [BigInt(wSnap)] });
      setWTx(tx);
    } catch (e: unknown) {
      setWErr((e as { shortMessage?: string }).shortMessage?.slice(0, 140) || "Failed");
    } finally { setWBusy(false); }
  };

  const handleCancelClick = () => {
    setCnErr("");
    const id = cnIdVal.current.trim();
  ...
  const doCancel = async () => {
    ...
    setCnBusy(true);
    try {
      const tx = await writeContractAsync({ address: FLUX_ADDRESS as `0x${string}`, abi: FLUX_ABI, functionName: "cancelStream", args: [BigInt(cnSnap)] });
      setCnTx(tx);
    } catch (e: unknown) {
      setCnErr((e as { shortMessage?: string }).shortMessage?.slice(0, 140) || "Failed");
  ...
  return (
    <div style={{ maxWidth: 1120, margin: "0 auto", padding: "32px 24px" }}>
      ...
      {confirm && (
        <ConfirmModal title="Create Payment Stream"
          message={<div><p style={{ marginBottom: 12 }}>Confirm stream:</p><div style={{ background: "var(--bg3)", borderRadius: 9, padding: "14px 16px", fontFamily: "'IBM Plex Mono',monospace", fontSize: 12 }}><div style={{ marginBottom: 5, wordBreak: "break-all" }}><span style={{ color: "var(--tx3)" }}>To: </span><span style={{ color: "var(--tx)" }}>{snap.recip}</span></div><div style={{ marginBottom: 5 }}><span style={{ color: "var(--tx3)" }}>Amount: </span><span style={{ color: "var(--teal)", fontWeight: 700 }}>${parseFloat(snap.amount).toFixed(2)} USDC</span></div><div style={{ marginBottom: 5 }}><span style={{ color: "var(--tx3)" }}>Start: </span><span>{snap.start}</span></div><div><span style={{ color: "var(--tx3)" }}>End: </span><span>{snap.end}</span></div></div><p style={{ marginTop: 10, fontSize: 12, color: "var(--tx3)" }}>USDC locked and released linearly over the stream duration.</p></div>}
          confirmLabel="Create Stream" onConfirm={doCreate} onCancel={() => setConfirm(false)} />
      )}
      {wConfirm && (
        <ConfirmModal title="Withdraw Vested USDC" message={<p>Claim all vested USDC from stream <strong>#{wSnap}</strong> to your wallet.</p>} confirmLabel="Withdraw" onConfirm={doWithdraw} onCancel={() => setWConfirm(false)} />
      )}
      {cnConfirm && (
        <ConfirmModal title="Cancel Stream" danger message={<p>Cancel stream <strong>#{cnSnap}</strong>. Vested USDC → recipient. Unvested USDC → you. Cannot be undone.</p>} confirmLabel="Cancel Stream" onConfirm={doCancel} onCancel={() => setCnConfirm(false)} />
      )}
      ...
      {tab === "create" ? (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 7 }}>
                    <label className="lbl" style={{ marginBottom: 0 }}>Recipient Address</label>
                    <Tooltip text="The wallet address that will receive the streamed USDC." />
                  </div>
                  {/* KEY FIX: onChange stores value in ref immediately */}
                  <input
                    className="inp"
                    placeholder="0x..."
                    onChange={e => { recipVal.current = e.target.value; setCreateErr(""); }}
                  />
                </div>
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 7 }}>
                    <label className="lbl" style={{ marginBottom: 0 }}>Total USDC Amount</label>
                    <Tooltip text="Total USDC to stream. Locked in contract and released linearly." />
                  </div>
                  <input
                    className="inp"
                    placeholder="1000.00"
                    type="number" min="0" step="0.01"
                    onChange={e => { amountVal.current = e.target.value; setPreviewAmount(e.target.value); setCreateErr(""); }}
                  />
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  <div>
                    <label className="lbl">Start Date</label>
                    <input className="inp" type="date" onChange={e => { startVal.current = e.target.value; setPreviewStart(e.target.value); setCreateErr(""); }} />
                  </div>
                  <div>
                    <label className="lbl">End Date</label>
                    <input className="inp" type="date" onChange={e => { endVal.current = e.target.value; setPreviewEnd(e.target.value); setCreateErr(""); }} />
                  </div>
                </div>
                ...
                {cTx ? (
                  <TxBanner hash={cTx} loading={cConf} explorerUrl={explorerLink("tx", cTx)} />
                ) : (
                  <button className="btn btn-primary btn-full" onClick={handleCreateClick} disabled={!authenticated || busy}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div className="card">
              <div className="card-hd">
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <span style={{ fontFamily: "'Manrope',sans-serif", fontSize: 14, fontWeight: 800, color: "#10b981" }}>Withdraw Vested</span>
                </div>
              ...
              <div className="card-p">
                ...
                <div style={{ display: "flex", gap: 10, marginBottom: 8 }}>
                  <input className="inp" placeholder="Stream ID (e.g. 0)" type="number" min="0" onChange={e => { wIdVal.current = e.target.value; setWErr(""); }} style={{ flex: 1 }} />
                  <button className="btn btn-primary btn-sm" onClick={handleWithdrawClick} disabled={!authenticated || wBusy} style={{ background: "#10b981", flexShrink: 0 }}>
                    {wBusy ? "…" : "Withdraw"}
                  </button>
                </div>
            ...
            <div className="card">
              <div className="card-hd">
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ width: 24, height: 24, borderRadius: 6, background: "var(--red-10)", border: "1px solid rgba(239,68,68,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12 }}>✕</div>
                  <span style={{ fontFamily: "'Manrope',sans-serif", fontSize: 14, fontWeight: 800, color: "var(--red)" }}>Cancel Stream</span>
                </div>
              ...
              <div className="card-p">
                ...
                <div style={{ display: "flex", gap: 10, marginBottom: 8 }}>
                  <input className="inp" placeholder="Stream ID (e.g. 0)" type="number" min="0" onChange={e => { cnIdVal.current = e.target.value; setCnErr(""); }} style={{ flex: 1 }} />
                  <button className="btn btn-danger btn-sm" onClick={handleCancelClick} disabled={!authenticated || cnBusy} style={{ flexShrink: 0 }}>
                    {cnBusy ? "…" : "Cancel"}
                  </button>
                </div>
              ...
              <div className="card-p">
                {[["Locked","USDC held in contract on creation"],["Vesting","amount × elapsed ÷ duration"],["Withdraw","Recipient claims anytime"],["Cancel","Sender recovers unvested USDC"],["Use cases","Payroll, grants, token vesting"]].map(([t,d]) => (
                  <div key={t} style={{ display: "flex", gap: 12, marginBottom: 8 }}>
                    <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, fontWeight: 700, color: "var(--teal)", flexShrink: 0, minWidth: 64, marginTop: 2 }}>{t}</span>
        ...
        <div className="card" style={{ overflow: "hidden" }}>
          ...
          {history.length === 0 ? (
            <EmptyState icon="⚡" title="No streams yet" desc="Create your first stream and it will appear here. IDs shown for withdraw/cancel." action={<button className="btn btn-primary btn-sm" onClick={() => setTab("create")}>Create a stream</button>} />
          ) : (
                    ...
                    <tr key={i}>
                      ...
                      <td>
                        <div style={{ display: "flex", gap: 6 }}>
                          <button className="btn btn-ghost btn-sm" onClick={() => { wIdVal.current = h.id.toString(); setWSnap(h.id.toString()); setTab("create"); }}>Withdraw</button>
                          <button className="btn btn-danger btn-sm" onClick={() => { cnIdVal.current = h.id.toString(); setCnSnap(h.id.toString()); setTab("create"); }}>Cancel</button>
                        </div>
                      </td>
                    </tr>
                  ))}
