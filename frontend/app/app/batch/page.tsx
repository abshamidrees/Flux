...
function FieldError({ msg }: { msg: string }) {
  return <div style={{ fontSize: 12, color: "#fca5a5", fontWeight: 600, marginTop: 6, display: "flex", alignItems: "center", gap: 5 }}><span>⚠</span> {msg}</div>;
}
  ...
  const doSettle = async () => {
    ...
    try {
      ...
      if (usdcBalance < totalNeeded) {
        setFormErr(`Insufficient USDC. Have ${formatUSDC(usdcBalance)}, need ${formatUSDC(totalNeeded)} USDC.`);
    ...
    <div style={{ maxWidth: 1120, margin: "0 auto", padding: "32px 24px" }}>
      ...
      <div style={{ marginBottom:22, display:"flex", justifyContent:"space-between", alignItems:"flex-end" }}>
        <div>
          ...
          <p style={{ fontSize:13, color:"var(--tx3)", fontWeight:500 }}>Send USDC to up to 500 recipients in one transaction. <Tooltip text="Flux charges 0.1% of the total USDC settled as a platform fee.">0.1% fee</Tooltip></p>
        </div>
        ...
        <div className="card" style={{ overflow:"hidden" }}>
          ...
          {loadingHistory ? (
            ...
            <div style={{ padding:"24px" }}>
              <div className="banner err" style={{ marginBottom:12 }}>{historyError}</div>
              <button className="btn btn-ghost btn-sm" onClick={fetchHistory}>Try again</button>
            </div>
          ) : history.length === 0 ? (
            <EmptyState icon="📋" title="No batch history yet" desc="Your settled batches appear here, live from the blockchain." />
          ) : (
